...
export default function Construct() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const constructData = await fetchConstructDataList();
      setConstruct(constructData);
    };
    fetchData();
  }, [page, like]);

  const fetchCurrentConstructCount = async () => {
    const res = await fetch(
      process.env.NEXT_PUBLIC_API_URL +
        `construct/current?limit=50&page=${page}`,
      {
        method: "GET",
      }
    );
  ...
  const fetchConstructDataList = async () => {
    const res = await fetch(
      process.env.NEXT_PUBLIC_API_URL +
        `construct?limit=50&page=${page}&like=${like}`,
      {
        method: "GET",
      }
    );
  ...
  const refreshData = async () => {
    const constructData = await fetchConstructDataList();
    const constructCount = await fetchCurrentConstructCount();
    setConstruct(constructData);
    if (constructCount.count.waiting === 0) setFinishPercent(100);
    else if (constructCount.count.finish === 0) setFinishPercent(0);
    else {
      setFinishPercent(
        (
          (constructCount.count.finish / (constructCount.count.waiting+constructCount.count.finish)) * 100
        ).toFixed(2)
      );
    }
  };
  ...
  const increasePage = () => {
    setPage(page >= construct.allPage ? construct.allPage : page + 1);
  };
  const decreasePage = () => {

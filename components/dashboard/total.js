...
export default function Total() {
  ...
  const [totalChartFilter, setTotalChartFilter] = useState("เดือน");
  ...
  useEffect(() => {
    ...
    if (!totalChartInstance.current) {
      ...
      if (totalChartFilter === "เดือน" && statement?.total) {
        for (let i = 0; i <= 30; i++) {
          label.push(
            `${currentTime.getDate()}/${
          ...
          try {
            if (
              statement["total"][currentTime.getFullYear()][
                currentTime.getMonth() + 1
      ...
      } else if (totalChartFilter === "ปี" && statement?.total) {
      ...
      label.reverse();
      data.reverse();
      setTotalSum(data.reduce((a, b)=>a+b, 0))
      totalChartInstance.current = new Chart(totalChartRef.current, {
        type: "line",
        data: {
          labels: label,
          datasets: [
  ...
  }, [totalChartFilter, statement]);

  const changeTotalChartFilter = () => {
    setTotalChartFilter(
      filterPool[(filterPool.indexOf(totalChartFilter) + 1) % 3]
    );
  };
    ...
    <div className="w-full h-full border border-black rounded-2xl bg-white">
      <div className="w-full h-[10%] flex place-content-between place-items-center pl-5">
        ...
        <div className="flex h-full place-items-center">
          <button
            onClick={changeTotalChartFilter}
      ...
      <div className="w-full h-[30%] bg-white flex place-content-start place-items-center">
        <h1 className={`font-['Sarabun'] font-bold text-[80px] ${totalSum >= 0 ? "text-green-500": "text-red-600"}`}>
          ฿{" "}
      ...
      <div className="w-full h-[60%] flex place-items-center place-content-center">
        <canvas className="w-full h-full" ref={totalChartRef}></canvas>
      </div>
    </div>
  );
}
